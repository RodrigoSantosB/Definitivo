...
int main() 
{
    pacoteCliente[0].posyPerso = 455;
    pacoteCliente[0].posxPerso = 265;
   ...
   // printf("Digite seu ID: ");
    ...
    strcpy(pacoteServidor.mensagem, msg);
    pacoteServidor.valor = 42;
...
//login IP
    ...
    while(lobby)
    {
        ...
        int rec = recvMsgFromServer(&pacoteServidor, DONT_WAIT);

        //recebe as mensagens do servidor (jogador se conectou, avisos, etc)
        if(rec != NO_MESSAGE){ 

                //printf("RECEIVED: %s\n", pacoteServidor.mensagem);

                for(i = 0; i < MAX_LOG_SIZE - 1; ++i)
                    strcpy(chatLog[i], chatLog[i+1]);
                
                strcpy(chatLog[MAX_LOG_SIZE - 1], pacoteServidor.mensagem);
                
                pacoteServidor.mensagem[0]='\0';
        }
        ...
        while(!al_is_event_queue_empty(eventsQueue))
        {
            ...
            if (lobbyEvent.type == ALLEGRO_EVENT_KEY_DOWN)
            {
                ...
                {
                    case ALLEGRO_KEY_ENTER:
                        strcpy(pacoteServidor.mensagem, lobbyMessage);
                        sendMsgToServer(&pacoteServidor, sizeof(ServerPack));
                        lobbyMessage[0]='\0';
                        break;
                }
            }
...
void printConnectScreen(char str[])
{
    ...
    al_draw_text(start, al_map_rgb(165,42,42), WIDTH / 2, 150, ALLEGRO_ALIGN_CENTRE, "Digite o IP do server:");

    if (strlen(str) > 0)
    {
        al_draw_text(start, al_map_rgb(165,42,42), WIDTH / 2,
                     (HEIGHT - al_get_font_ascent(start)) / 1.6,
...
void printLoginScreen(char str[])
{
    ...
    al_draw_text(start, al_map_rgb(165,42,42), WIDTH / 2, 150, ALLEGRO_ALIGN_CENTRE, "Digite o Login desejado: ");
